...
Q1(1,1) = 2*1;                          % Weight on state x1, 
Q1(2,2) = 0;                            % Weight on state x2
Q1(3,3) = 0;                            % Weight on state x3
Q1(4,4) = 0;                            % Weight on state x4
P1 = 10;                                 % Weight on input, THIS IS q in forarbeid
Q = gen_q(Q1,P1,N,M);                   % Generate Q, hint: gen_q
c = zeros(N*mx+M*mu,1);                 % Generate c, this is the linear constant term in the QP

%% Generate system matrixes for linear model
Aeq = gen_aeq(A1,B1,N,mx,mu);             % Generate A, hint: gen_aeq
% beq = [A1*x0;0;0;0];% Generate b
beq = A1*x0;
beq(5:400) = zeros(396,1);
...
x_timeTable = timetable(x1,x2,x3,x4,'TimeStep',seconds(0.25));

toSimulink = [u_timeTable, x_timeTable];
